 ...
 */
async function getModuleTopics(moduleId: string) {
  const result = await pool.query(
    `SELECT
      t.id,
      t.title,
 ...
 */
async function getStrugglingStudents(threshold: number = 3) {
  const result = await pool.query(
    `SELECT
      u.id,
      u.email,
 ...
 */
async function getRecentSubmissions(limit: number = 10) {
  const result = await pool.query(
    `SELECT
      s.id,
      s.user_id,
...
server.setRequestHandler(async (request) => {
  ...
  if (name === "tools/call") {
    const { name: toolName, arguments } = request.params;

    try {
      switch (toolName) {
        case "get_user_by_id":
          const user = await getUserById(arguments.userId as string);
          return {
            content: [{
              type: "text",
        ...
        case "get_progress_summary":
          const summary = await getProgressSummary(arguments.studentId as string);
          return {
            content: [{
              type: "text",
        ...
        case "get_module_topics":
          const topics = await getModuleTopics(arguments.moduleId as string);
          return {
            content: [{
              type: "text",
        ...
        case "get_struggling_students":
          const struggling = await getStrugglingStudents(
            arguments.threshold as number || 3
          );
          return {
            content: [{
              type: "text",
